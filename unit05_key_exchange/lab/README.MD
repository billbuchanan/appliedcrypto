...
Bob Select b=6, Alice selects a=9

Now calculate (using a calculator or Python:

Alice’s A value (A=g<sup>a</sup> mod N):	
	
Bob’s B value (B=g<sup>b</sup> mod N):

Note: In Python, we can use A=pow(g,a,p)

### A.2	
Now they exchange the values. Next calculate the shared key:


Alice’s value (key=B<sup>a</sup> mod N):	
	
Bobs’s value (key=A<sup>b</sup> mod N):
...
### B.3 Now we will implement a key exchange in OpenSSL with ECDH. Implement the following, and check that the derived keys are the same. First generate Alice's private and public keys:
...
Alice generates derived key:
```
% openssl pkeyutl -derive -inkey alice_private.pem -peerkey bob_public.pem -out shared_secret.bin
```
and Bob does the same:
```
Bob generates derived key
% openssl pkeyutl -derive -inkey bob_private.pem -peerkey alice_public.pem -out shared_secret2.bin
```

and then we can check the derived keys to see if they are the same:
```
% xxd shared_secret.bin
00000000: e92e c12b 608c 223e d17a 93b7 1758 f9c7  ...+`.">.z...X..
00000010: 1d99 b760 6302 860e ca6e b031 8ac1 a034  ...`c....n.1...4
```
and:
```
